...
- **Global scope**: `C:/PTOW/1_APT_Central_Command/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/PTOW/1_APT_Central_Command/.claude/notes/`)
...
2. Glob `C:/PTOW/1_APT_Central_Command/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/PTOW/1_APT_Central_Command/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/PTOW/1_APT_Central_Command/.claude/notes/` — works in any directory
