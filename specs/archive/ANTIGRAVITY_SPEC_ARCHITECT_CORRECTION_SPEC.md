# ANTIGRAVITY_SPEC_ARCHITECT_CORRECTION_SPEC.md
# sentinel-spec-architect: Rewrite to webhook HTTP server
# Sprint 31.1 | Spec author: Claude Code | Date: 2026-04-28

---

## Context

AG shipped `sentinel-spec-architect` as a GitHub issue poller (watches for `spec-request` labels).
The spec requires an HTTP webhook server that responds to `POST /spec` requests.
The shipped version also uses the wrong Claude model and wrong token limit.

**Three deviations to fix:**
1. Architecture: GitHub issue poller → HTTP webhook server
2. Model: `claude-opus-4-5` → `claude-opus-4-7`
3. max_tokens: `4096` → `8000`

---

## File to Replace

**`railway/sentinel-spec-architect/index.js`** — replace the entire file with the code below.
Do not touch any other file. `package.json` and `Dockerfile` are correct and must not change.
 ...
 *   GITHUB_REPO          — owner/repo (e.g. White-Jesus/central-command)
...
async function fetchClaudeMd() {
  const res = await fetch(`https://raw.githubusercontent.com/${GITHUB_REPO}/main/CLAUDE.md`);
...
## What Changed vs. Shipped Code

| | Shipped (wrong) | Correct |
|---|---|---|
| Architecture | `setInterval` polling GitHub issues | HTTP server, `POST /spec` |
| Trigger | GitHub issue labelled `spec-request` | JSON body `{ key, goal }` |
| Auth | None (GitHub's label system) | `SPEC_ARCHITECT_KEY` check |
| Model | `claude-opus-4-5` | `claude-opus-4-7` |
| max_tokens | `4096` | `8000` |
| System prompt | Inline in user message | Separate `system` field |

---

## What Must NOT Change

- `railway/sentinel-spec-architect/package.json` — correct, do not touch
- `railway/sentinel-spec-architect/Dockerfile` — correct, do not touch
- All other sentinel files — correct, do not touch
- `dashboard-api/DashboardAPI.gs` — correct, do not touch

---

## Verification

After Railway redeploys the service:

```bash
curl -X POST https://[railway-spec-architect-url]/spec \
  -H "Content-Type: application/json" \
  -d '{"key":"[SPEC_ARCHITECT_KEY]","goal":"Add a tech utilization chart to the /team page"}'
```

Expected: `{"status":"GENERATING","message":"Spec generation started. PR will be opened when complete."}`

Within ~60 seconds: a new PR should appear at `https://github.com/White-Jesus/central-command/pulls`
with a `ANTIGRAVITY_ADD-A-TECH-UTILIZATION-CHART_SPEC.md` file.

Wrong key → `{"error":"UNAUTHORIZED"}` (401)
Missing goal → `{"error":"MISSING_GOAL"}` (400)
Any other path → 404
